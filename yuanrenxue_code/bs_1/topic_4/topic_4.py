...
def req(page):
    ...
    # data = json.loads(open('topci_4.json').read())
    data = response.json()
    info = data['info']

    # s_b64 = base64.b64encode('We8z1Nfbri3Ano1x2qa5'.encode()).decode()
    s_b64 = base64.b64encode((data['key'] + data['value']).encode()).decode().replace('=', '')

    class_id = hashlib.md5(s_b64.encode()).hexdigest()
    return class_id,info
# print(class_id)


def id_img_number(c_id,number):
    _number = number.replace('img_number ', '')
    _ = True if c_id in _number else False
    # print(_number,_)
    return _


def caculate_css_laft(number_style_list,number_list):
    print("number_style_list:",number_style_list)
    print("number_list:",number_list)
    _number_style_list = [round(int(float(_)) / 11) for _ in number_style_list]
    # print(_number_style_list,number_style_list)
    ture_order_list = [None] * len(number_list)
    for index,number_style in enumerate(_number_style_list):
        # print(number_style,index,index + number_style,int(index + number_style))
        ture_order_list[int(index + number_style)] = number_list[index]
    print(ture_order_list)
    return ture_order_list


def call_number_tb(class_id,info):
    sel = Selector(text=info)
    number = 0
    for td in sel.css('td'):
        number_list = []
        number_style_list = []
        for img in td.css('img'):
            if id_img_number(class_id, img.css("::attr(class)").get()):
                continue
            # print(img)
            number_list.append(img_map[img.css("::attr(src)").get()])
            number_style_list.append(img.css('::attr(style)').get().replace('left:', '').replace('px', ''))

        # print(number_list, number_style_list)
        # print(caculate_css_laft(number_style_list,number_list))
        num = "".join([str(i) for i in caculate_css_laft(number_style_list, number_list)])
        number += int(num)
    return number
...
def _test():
    ...
    class_id = hashlib.md5(s_b64.encode()).hexdigest()
    print(call_number_tb(class_id, info))

if __name__ == '__main__':
    # number = 0
    # for i in range(1,6):
    #     c_id,info = req(i)
    #     number += call_number_tb(c_id,info)
    # print(number)
    _test()
